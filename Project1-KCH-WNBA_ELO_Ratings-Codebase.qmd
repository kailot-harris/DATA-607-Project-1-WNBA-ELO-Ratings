...
summary_2025 <- all_teams_df |>
  ...
  summarize(total_games = n(),
            total_wins = sum(win),
            total_losses = total_games - total_wins, #not an independent column, nice for summary
            final_elo = last(team_elo_post),
            avg_opp_elo = mean(opp_elo_pre)
            ) |>
...
summary_2025 |> ggplot(aes(x=final_elo, y=total_wins)) +
...
- ungrouped smooth line?

```{r viz-2}
top_5_teams_df <- all_teams_df |>
  filter(team %in% c(""))
