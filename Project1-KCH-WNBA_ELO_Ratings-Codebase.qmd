...
knitr::kable(tmp, caption = "Check for Duplicate Games")

tmp <- full_df |>
  filter(
    (home_team_display_name == "Minnesota Lynx" & away_team_display_name == "Indiana Fever") |
      (home_team_display_name == "Indiana Fever" & away_team_display_name == "Minnesota Lynx")
  ) |>
  select(game_id, game_date, home_team_display_name, away_team_display_name, home_win)

knitr::kable(tmp, caption = "Check for Extra Games")
```

## Calculating Running Elo Ratings

A running Elo rating is calculated using the [Elo R package](https://cran.r-project.org/web/packages/elo/index.html) with documentation here: . The `elo.run()` function will be used, which is documented in this [specific vignette](https://cran.r-project.org/web/packages/elo/vignettes/running_elos.html). The initial K value has been set to a value of 50. In the github article above, the author noted that varying K between 20 and 50 did not fundamentally alter the accuracy of their Elo-based predictions. Since the WNBA season is relatively short (44 games) compared to the NBA (82 games), a high, stable K value is preferred so that the Elo ratings are more responsive to intra-season momentum. The baseline Elo rating for the beginning of the 2025 season was set to 1300, as is convention for NBA Elo ratings analysis. Future work could perform experiments where these two parameters are tuned to find optimal settings for predictive accuracy.
...
First, a plot showing the linearity of a Total Wins vs. Elo Ratings is constructed. Here, custom formatting is achieved using the `theme()` function.
...
upsets |> ggplot(aes(x=elo_diff, y=reorder(matchup, elo_diff))) +
  ...
  geom_text(aes(label = paste("+", round(elo_diff, 0), " pts")), 
            hjust = 1.15, size = 4, fontface = "bold") +
